...
if ( [ $# -ne 1 ] ) then
	echo "Usage: ./build_and_deploy_in_prod.sh <PROJECT_ROOT>"
	exit -1
else
  PROJECT_ROOT="$1"
  ...
  ${PROJECT_ROOT}/Scripts/generate_prod_appcast.sh "${TAR_NAME}" "${VERSION}" "${BUILD_NUMBER}" "${SIGNATURE}" "${OUTPUT_APPCAST}"
